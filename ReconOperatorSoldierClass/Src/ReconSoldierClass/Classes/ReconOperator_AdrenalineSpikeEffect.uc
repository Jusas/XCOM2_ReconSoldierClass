class ReconOperator_AdrenalineSpikeEffect extends X2Effect_Persistent;
/*
function RegisterForEvents(XComGameState_Effect EffectGameState)
{
	...
	EventMgr.RegisterForEvent(EffectObj, 'UnitTakeEffectDamage', static.OnDamageTaken, ELD_OnStateSubmitted);
}
...
static function EventListenerReturn OnDamageTaken(Object EventData, Object EventSource, XComGameState GameState, Name EventID)
{
	...
	foreach HitUnit.AffectedByEffects(ObjectRef)
	{
		...
		if(EffectState.GetX2Effect().IsA('ReconOperator_AdrenalineSpikeEffect'))
		{
			`log("[ReconOperator]-> Got hit, adding an action point.");
